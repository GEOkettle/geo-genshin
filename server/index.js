...
//   return config;
// });
const createDataMachine = (cookie, ltuid) => {
  const headers = {
    "User-Agent":
      "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69 Safari/537.36",
    Cookie: cookie,
  ...
  const dataMachine = axios.create({ headers });

  dataMachine.interceptors.request.use((config) => {
    config.headers.DS = generateDSToken();
    config.headers.Cookie = cookie;
    config.headers.ltuid = ltuid;
    return config;
  });
...
console.log(req.body)
    let ltuid = req.body.uid || BASIC_UID;
    console.log(ltuid)
...
const cookie = `_MHYUUID=${getUuid()}; G_ENABLED_IDPS=google; _ga=GA1.2.945851836.1668241513; _gid=GA1.2.617282950.1673273808;  ltoken=${LTOKEN}; ltuid=${ltuid}; cookie_token=${CTOKEN}; account_id=${ACCID}; mi18nLang=ko-kr;`;
 const dataMachine = createDataMachine(cookie, ltuid)
...
(async () => {
  ...
  const weaponID = allWeaponList.find(
    (w) => w.name === req.body.weaponName
  ).entry_page_id;
  const weaponDetail = await dataMachine2
    .get(
...
app.post("/currency", (req, res) => {
  ...
  (async () => {
   ...
   const currentAbyss = await dataMachine
     .get(
       `https://bbs-api-os.hoyolab.com/game_record/genshin/api/spiralAbyss?role_id=${gameRoleId}&server=${server}&schedule_type=1`, {
    role_id: gameRoleId,
    server:  server,
    schedule_type: "1"}
     )
   ...
   const lastAbyss = await dataMachine
     .get(
       `https://bbs-api-os.hoyolab.com/game_record/genshin/api/spiralAbyss?role_id=${gameRoleId}&server=${server}&schedule_type=2`, {
    role_id: gameRoleId,
    server:  server,
    schedule_type: "2"}
     )
    ...
    //sg-hk4e-api.hoyolab.com/event/ysledgeros/month_info?month=1&region=os_asia&uid=852616698&lang=ko-kr
    // const abyss = await dataMachine
    //   .get(
    //     `https://bbs-api-os.hoyolab.com/api/game/genshin/abyss/upload?uid==165181087`
    //   )
